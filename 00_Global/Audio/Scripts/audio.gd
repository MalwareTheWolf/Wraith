extends Node
# Audio global script.
# Handles music playback, UI sounds, reverb, spatial SFX,
...
@export var ui_focus_audio: AudioStream = preload("uid://c5u1usp4cn751")
@export var ui_select_audio: AudioStream = preload("uid://8o4dlxrbttkl")
...
func setup_button_audio(node: Node) -> void:
	for c in node.find_children("*", "Button"):
		...
		if not button.mouse_entered.is_connected(ui_focus_change):
