...
#PLAYER
#Handles movement, state changes, combat, abilities, health, footsteps, and player animations.
...
func get_surface_type() -> String:
	...
	var map_pos: Vector2i = ground_tilemap.local_to_map(
		ground_tilemap.to_local(check_pos)
	)
...
func play_save_animation() -> void:
	if animation_player == null:
		return

	if animation_player.has_animation("Save"):
